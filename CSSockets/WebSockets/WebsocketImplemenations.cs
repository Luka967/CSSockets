...
namespace CSSockets.WebSockets
{
    public class ServerWebSocket : WebSocket
    {
        ...
        {
            lock (OpsLock) Send(new Frame(true, 9, false, data ?? new byte[0], false, false, false));
        }
        public override void Send(byte[] data)
        {
            lock (OpsLock) Send(new Frame(true, 2, false, data, false, false, false));
        }
        public override void Send(string data)
        {
            lock (OpsLock) Send(new Frame(true, 1, false, Encoding.UTF8.GetBytes(data), false, false, false));
        }
        ...
        {
            lock (OpsLock) Send(new Frame(true, 10, false, data, false, false, false));
        }
    }
    ...
    public class ClientWebSocket : WebSocket
    {
        ...
        public override void Ping(byte[] data = null)
        {
            lock (OpsLock) Send(new Frame(true, 9, true, data ?? new byte[0], false, false, false));
        }
        public override void Send(byte[] data)
        {
            lock (OpsLock) Send(new Frame(true, 2, true, data, false, false, false));
        }
        public override void Send(string data)
        {
            lock (OpsLock) Send(new Frame(true, 1, true, Encoding.UTF8.GetBytes(data), false, false, false));
        }
        ...
        protected override void AnswerPing(byte[] data)
        {
            lock (OpsLock) Send(new Frame(true, 10, true, data, false, false, false));
        }
    }
}
